...
class ServerWorker:
	...
	def processRtspRequest(self, data):
		...
		# Process REC request
		elif requestType == self.REC:           
			if self.state == self.PLAYING:  
				print("processing REC\n")
				self.state = self.PLAYING
				self.recframe = self.clientInfo['videoStream'].frameNbr()
				self.replyRtsp(self.OK_200, seq[1])

		# Process Play REC request
		elif requestType == self.PLAYREC:           
			if self.state == self.PLAYING:  
				print("processing PLAYREC\n")
				self.state = self.PLAYING
				if self.recframe != -1:
					self.rec = 1
	...
	def sendRtp(self):
		...
		while True:
			...
			data = self.clientInfo['videoStream'].nextFrame(self.forward, self.backward, self.clientInfo['videoStream'].frameNbr(), self.recframe, self.rec)
			if (self.backward == 1): self.backward = 0
			if (self.forward == 1): self.forward = 0
			if (self.rec == 1): self.rec = 0
			if data: 
	...
	def replyRtsp(self, code, seq, req = '', file = ''):
		...
		if code == self.OK_200:
			if req == 'DESCRIBE':
				sessionInfo =  '\nv = 2' 
				sessionInfo += '\ns = ' + str(self.clientInfo['session']) 
				sessionInfo += '\no = ' + str(self.clientInfo['rtspSocket'][1][0])  
				sessionInfo += '\nm = Video ' + file + ' RTP/Mjpeg'
				sessionInfo += '\na = Encode utf-8'
				sessionInfo += '\na = Size ' +  str(self.clientInfo['videoStream'].getSize()) + ' Bytes' 				
